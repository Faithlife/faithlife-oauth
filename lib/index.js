...
function subapp(app) {
    ...
    return makeRequest({
      ...
      headers: {
        Authorization: generateAuthorization({
          oauth_callback: 'http://' + request.headers.host + '/oauth/verify'
        })
      }
    })
      .then(function (response) {
        console.log('temporarytoken response: %j', response.entity);
  ...
  app.get('/verify', function (request) {
    return makeRequest({
      ...
      .then(function (response) {
        console.log('accesstoken response: %j', response.entity);
        request.session.oauth_token = response.entity.oauth_token;
        request.session.oauth_token_secret = response.entity.oauth_token_secret;
        return mach.redirect('/oauth/me');
      });
  });
  ...
  app.get('/me', function (request) {
    return makeRequest({
      method: 'GET',
      path: 'https://accountsapi.logos.com/v2/users/me',
      headers: {
        Authorization: generateAuthorization({
          oauth_token_secret: request.session.oauth_token_secret,
      ...
      .then(function (response) {
        console.log('me response: %j', response.entity);
        return mach.json(response.entity);
      });
  });
}
